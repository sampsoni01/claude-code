  ...
  const LIB = [
    ...
    /* --------------------------------- follow-ups scheduled by an option */
    {
      ...
      brief: (st) => `<p>The international tribunal has ruled on the expropriation we carried out ${ago(st, 'nationalisation')}. It has found against us and assessed damages at ${S.money(st.economy.gdp * 0.022)}.</p>
        <p>Paying is expensive. Not paying puts every foreign asset we hold abroad within reach of enforcement.</p>`,
      advisors: () => [
        { who: 'Attorney General', role: 'Justice', said: 'The award is enforceable in forty jurisdictions. They will start with the aircraft.' },
        { who: 'Nationalist Caucus', role: 'Legislature', said: 'A foreign tribunal has no authority over what we do with our own ground.' }
      ],
      options: [
        { label: 'Pay the award in full', detail: 'Settle it and restore standing.',
        ...
        { label: 'Negotiate a payment schedule', detail: 'Instalments, over a decade.',
          effects: { 'economy.businessConfidence': +3 },
          fn: (st) => { st.economy.debt += st.economy.gdp * 0.022; }, factions: { business: +4 } },
        { label: 'Refuse to recognise the ruling', detail: 'Sovereignty over solvency.',
    ...
    },
    {
      ...
      brief: (st, ctx) => {
        ...
        return `<p>The complex we took into public ownership ${ago(st, 'nationalisation')} has now had four years under state management.</p>
          <p>${ctx.good ? 'Output is up, the workforce has grown, and it is returning a dividend to the treasury. This is not the usual outcome and the ministry would like you to notice.'
            : 'Output is down a third, the maintenance backlog is enormous, and it has not paid a dividend since the day we took it. This is the usual outcome.'}</p>`;
      },
      options: [
        { label: 'Reinvest and expand it', detail: 'Double down on state ownership.',
        ...
        { label: 'Bring in private management under state ownership', detail: 'Their people, our shares.',
          effects: { 'economy.businessConfidence': +5, 'quality.admin': +2, 'economy.shock': +0.2 },
          factions: { business: +7, labour: -5 } },
        { label: 'Sell it back', detail: 'Admit it and take the money.',
    ...
    },
    {
      ...
      brief: (st, ctx) => {
        ...
        return `<p>${n ? n.name : 'Our ally'} has been attacked and has invoked the defence pact we signed. The text is unambiguous. Our obligation is unambiguous. Whether we honour it is not.</p>
          <p>Our force index is ${S.round(st.military.power, 1)}; readiness ${S.round(st.military.readiness, 0)}.</p>`;
      },
      advisors: () => [
        { who: 'Chief of Staff', role: 'Defence', said: 'We can deploy. I want you to understand what deploying means before you order it.' },
        { who: 'Foreign Minister', role: 'Diplomacy', said: 'An alliance you do not honour is not an alliance, and every other signature we hold becomes worthless the same afternoon.' }
      ],
      options: [
        { label: 'Honour it in full and deploy', detail: 'War, because we said we would.',
        ...
        { label: 'Send materiel, not troops', detail: 'Everything except the thing they asked for.',
        ...
        { label: 'Refuse and withdraw from the pact', detail: 'Say plainly that we will not come.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The appeal court has begun quashing convictions from the anti-corruption campaign of ${ago(st, 'corruption_purge')}. It finds the evidence in several cases was assembled to fit a conclusion.</p>
        <p>Corruption currently stands at ${S.round(st.society.corruption, 0)}/100 — lower than before the campaign, which is the awkward part.</p>`,
      options: [
        { label: 'Accept the judgments and compensate', detail: 'The rule of law includes the inconvenient bits.',
          effects: { 'quality.admin': +4, 'society.approval': -4, 'society.freedom': +4 },
          factions: { reformers: +8, intelligentsia: +9, business: +5 } },
        { label: 'Retry them properly', detail: 'Do it again, and do it correctly.',
          effects: { 'quality.admin': +2, 'society.corruption': -2, 'budget.alloc.interior': +0.1 },
          factions: { reformers: +5, business: -4 } },
        { label: 'Legislate to overturn the appeals', detail: 'Change the law retrospectively.',
    ...
    },
    {
      ...
      brief: (st, ctx) => {
        const n = ctx.nation ? S.dip(st, ctx.nation) : null;
        return `<p>Two servicemen from the ${n ? n.adj : 'foreign'} base killed a local woman with a vehicle and have been flown home under the status of forces agreement you signed ${ago(st, 'foreign_base_request')}.</p>
          <p>There have been crowds at the gates for three nights.</p>`;
      },
      options: [
        { label: 'Demand jurisdiction and renegotiate the agreement', detail: 'Our courts, our soil.',
        ...
        { label: 'Accept the agreement as written', detail: 'We signed it knowing this was in it.',
          effects: { 'society.approval': -8, 'society.unrest': +8, 'society.latent': +8 },
          factions: { nationalists: -14, military: +3 } },
        { label: 'Close the base', detail: 'End it, and the money with it.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The audit office reports the high-speed programme is running at nearly double its authorisation and four years behind. Half the route is a construction site and none of it carries passengers.</p>
        <p>It has become the single most-cited example of everything wrong with this government.</p>`,
      options: [
        { label: 'Fund it to completion', detail: 'A half-built railway is worth nothing at all.',
        ...
        { label: 'Descope it to the profitable section', detail: 'Half a railway, honestly described.',
        ...
        { label: 'Cancel it', detail: 'Stop, and write off everything spent.',
    ...
    },
    {
      ...
      brief: (st) => `<p>Among those released under the amnesty ${ago(st, 'amnesty')} was a man who has spent the intervening period building the largest opposition movement this country has seen.</p>
        <p>He has committed no crime since. Approval stands at ${S.round(st.society.approval, 0)}%.</p>`,
      options: [
        { label: 'Let him stand', detail: 'The amnesty meant what it said.',
        ...
        { label: 'Bar him on a technicality', detail: 'The registration rules are quite detailed.',
        ...
        { label: 'Re-arrest him', detail: 'Undo the amnesty in his case.',
        ...
        { label: 'Offer him a ministry', detail: 'Bring him inside where he can be blamed.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The war is over and we have a very large army with nothing to do. Demobilising it puts a great many trained young men into a labour market with ${S.round(st.economy.unemployment, 1)}% unemployment.</p>
        <p>Every state that has got this wrong has regretted it within five years.</p>`,
      options: [
        { label: 'Phased demobilisation with retraining', detail: 'Slow, funded, and the only version that works.',
        ...
        { label: 'Keep the force under arms', detail: 'Employment by another name.',
          effects: { 'budget.alloc.defense': +0.3, 'military.readiness': +4, 'economy.shock': -0.3 },
          factions: { military: +10, nationalists: +7, business: -6 } },
        { label: 'Immediate mass discharge', detail: 'Cheapest today.',
        ...
        { label: 'Convert them into a labour corps', detail: 'Put them on the infrastructure programme.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The lenders have arrived for the review of the programme we entered ${ago(st, 'debt_wall')}. They have a list of conditions we have not met and the next tranche depends on them.</p>
        <p>Debt stands at ${S.round(st.economy.debtGdp, 0)}% of output; the deficit at ${S.round((st.economy.deficit / st.economy.gdp) * 100, 1)}%.</p>`,
      options: [
        { label: 'Meet the conditions', detail: 'Cut, and take the money.',
        ...
        { label: 'Negotiate a slower path', detail: 'Same destination, more time.',
        ...
        { label: 'Leave the programme', detail: 'Take back the sovereignty and lose the money.',
    ...
    },
    {
      ...
      brief: (st, ctx) => {
        ...
        return `<p>The first full harvest since the land settlement of ${ago(st, 'land_reform')} is in.</p>
          <p>${ctx.good ? 'Output held and smallholder incomes are up sharply. The titling worked, which is not what the critics predicted.'
            : 'Output has fallen by a fifth. The new holdings are too small to mechanise and nobody can borrow against land they only half own.'}</p>`;
      },
      options: [
        { label: 'Extend credit and extension services', detail: 'Give the new owners what they lack.',
        ...
        { label: 'Encourage co-operatives', detail: 'Small holdings, pooled machinery.',
          effects: { 'economy.shock': +0.2, 'economy.productivity': +1 }, factions: { labour: +7, business: -3 } },
        { label: 'Allow consolidation to reverse it', detail: 'Let the market undo what the law did.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The measures imposed after the nuclear test are now fully in force. Sanctions pressure stands at ${S.round(st.economy.sanctionPressure, 0)}/100, the currency is at ${S.round(st.economy.fx, 0)}, and three of our banks have lost correspondent relationships.</p>
        <p>The deterrent is real. So is this.</p>`,
      options: [
        { label: 'Hold the posture and endure', detail: 'The arsenal was the point.',
          effects: { 'society.approval': -4, 'economy.shock': -0.5, 'society.cohesion': +5 },
          factions: { nationalists: +12, military: +8, business: -10 } },
        { label: 'Open talks on verification', detail: 'Trade transparency for relief.',
        ...
        { label: 'Renounce the programme entirely', detail: 'Give it up and get the economy back.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The pension question has now been deferred twice. The fund's assets no longer cover the coming year's obligations and the shortfall must be met from general revenue this quarter.</p>
        <p>There is no version of this that is not now painful, and the painless versions were the ones we declined.</p>`,
      options: [
        ...
        { label: 'Fund it from borrowing', detail: 'Move the problem into the debt stock.',
        ...
        { label: 'A dedicated pension levy', detail: 'A visible tax for a visible purpose.',
    ...
    },
    {
      ...
      brief: (st) => `<p>A second failure of exactly the kind the rejected inquiry warned about has occurred. The report we dismissed ${ago(st, 'disaster_inquiry')} is being read aloud on television, paragraph by paragraph.</p>`,
      options: [
        { label: 'Accept the original findings in full', detail: 'Late, and better than never.',
        ...
        { label: 'Dismiss the ministers who advised rejection', detail: 'Find someone else to have been wrong.',
          effects: { 'society.approval': +3, 'quality.admin': -3, 'society.corruption': +1 },
          factions: { reformers: -4, business: -3 } },
        { label: 'Say nothing at all', detail: 'The story has three days in it.',
    ...
    /* ------------------------- gated on state, not scheduled by an option */
    {
      ...
      brief: (st) => `<p>A delegation of foreign parliamentarians has arrived and asked, in a meeting that was supposed to be about trade, why press freedom in this country stands where it does. It stands at ${st.policy.interior.pressFreedom}/100.</p>
        <p>They have a report, our editors have briefed them, and they are holding a press conference on Thursday.</p>`,
      options: [
        { label: 'Release the detained journalists', detail: 'A gesture, and a real one.',
        ...
        { label: 'Give them a tour and a dinner', detail: 'Charm rather than change.',
          effects: { 'national.softPower': +2 }, factions: {} },
        { label: 'Expel the delegation', detail: 'This is an internal matter.',
    ...
    {
      id: 'surveillance_scandal', cat: 'civic', title: 'We Were Watching the Wrong People',
      from: 'Cabinet Secretary', urgency: 'pressing', deadline: 12,
      weight: (st) => (st.policy.interior.surveillance > 65 ? 15 : 0),
      brief: (st, ctx) => {
        const f = F(st, ctx);
        return `<p>Surveillance powers standing at ${st.policy.interior.surveillance}/100 have been used against opposition politicians, two judges and a delegation from ${f.union}. The authorisations were signed inside this building.</p>`;
      },
      options: [
        { label: 'Full disclosure and an independent regulator', detail: 'Publish the authorisations and hand over the keys.',
        ...
        { label: 'Discipline the officials who signed', detail: 'The powers were fine; the use was not.',
          effects: { 'policy.interior.surveillance': -6, 'quality.admin': +2, 'society.latent': +3 },
          factions: { reformers: +4, intelligentsia: +3 } },
        { label: 'Deny it', detail: 'The authorisations are classified and therefore did not happen.',
    ...
    {
      id: 'war_profiteering', cat: 'military', title: 'Somebody Is Getting Rich',
      from: 'National Audit Office', urgency: 'pressing', deadline: 14,
      weight: (st) => (st.wars.length && st.society.corruption > 30 ? 15 : 0),
      brief: (st, ctx) => {
        const f = F(st, ctx);
        return `<p>Wartime procurement has been running without competition for two years. ${f.firm} has been paid four times the pre-war rate for rations and the contracts were signed by ${f.official}.</p>
          <p>Soldiers at the front have noticed. That is the part the Chief of Staff wanted you to hear first.</p>`;
      },
      options: [
        { label: 'Prosecute, mid-war', detail: 'Arrests now, whatever it does to supply.',
        ...
        { label: 'Renegotiate the contracts quietly', detail: 'Recover the money without the spectacle.',
        ...
        { label: 'Leave it until the war ends', detail: 'Supply first. Justice later, if at all.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The people we admitted ${ago(st, 'refugee_flow')} now have children who have never known anywhere else, and who are not citizens of anywhere at all.</p>
        <p>Cohesion stands at ${S.round(st.society.cohesion, 0)}/100.</p>`,
      options: [
        { label: 'Citizenship by birth for all of them', detail: 'Settle it in one line of statute.',
        ...
        { label: 'Leave them stateless', detail: 'Their parents\' status, inherited.',
    ...
    },
    {
      ...
      brief: (st) => `<p>Veterans of the war have formed a movement and marched on the capital. Personnel and veteran support stands at ${st.policy.mil.veteranCare}/100, which they can quote from memory.</p>
        <p>They are disciplined, they are organised, and a great many of them are armed and know how to use it.</p>`,
      options: [
        ...
        { label: 'Partial settlement', detail: 'Enough to split the leadership.',
          effects: { 'policy.mil.veteranCare': +10, 'society.unrest': -3 }, factions: { military: +5 } },
        { label: 'Disperse them', detail: 'Police, against the men you sent to fight.',
    ...
    },
    {
      ...
      brief: (st) => {
        ...
        return `<p>${p ? p.name : 'The national programme'} has caught the public imagination in a way the ministry did not anticipate. It is on the news weekly and people have started naming things after it.</p>
          <p>There is an argument for spending that goodwill and an argument for banking it.</p>`;
      },
      options: [
        ...
        { label: 'Launch a second one on the strength of it', detail: 'Ride the goodwill into the next thing.',
        ...
        { label: 'Put your name on it', detail: 'Make the credit personal and permanent.',
    ...
    },
    {
      ...
      brief: (st) => {
        ...
        return `<p>The commemoration of ${s ? s.name : 'the disaster'} has been taken over by the bereaved families, who have invited the opposition and not you.</p>
          <p>${s ? S.num(s.deaths) + ' people died' : 'A great many died'}, and ${s ? S.round(s.severity, 0) : 'much'} per cent of the damage is still there to be photographed.</p>`;
      },
      options: [
        { label: 'Attend anyway and take the reception', detail: 'Stand there and be booed if necessary.',
          effects: { 'society.approval': +4, 'society.cohesion': +4, 'society.legitimacy': +4 },
          factions: { reformers: +6, provinces: +8 } },
        { label: 'Announce a compensation fund at the ceremony', detail: 'Turn up with money.',
        ...
        { label: 'Hold a rival official ceremony', detail: 'Two commemorations, competing on television.',
    ...
    },
    {
      ...
      brief: (st) => `<p>The smaller party in the coalition has told the press it is reviewing its position. Approval stands at ${S.round(st.society.approval, 0)}% and they can read a poll as well as anyone.</p>
        <p>Without them there is no majority and no budget.</p>`,
      options: [
        { label: 'Concede their policy demands', detail: 'Buy them with the programme.',
        ...
        { label: 'Give them the ministries they want', detail: 'Buy them with jobs.',
          effects: { 'quality.admin': -3, 'society.stability': +5, 'society.corruption': +2 }, factions: { reformers: -5 } },
        { label: 'Call their bluff', detail: 'Govern as a minority and dare them.',
        ...
        { label: 'Go to the country early', detail: 'An election now, on your timing rather than theirs.',
